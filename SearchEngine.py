...
class SearchEngine:
    ...
    def query(self, user_id, text, to_read_boosting_factor, K, save_res=True):
        ...
        """
        assert int(user_id) == user_id and 0 < user_id <= 53424, "Please input a valid user-id: integer between 1 and 53424 (inclusive)"
    ...
    def author_boost(self, text):
        ...
        for i in idx:
            '''if (p==1):
                p+=1
                print("Author is in your qurey")
                boost = float(input("Enter number between 1-50. \
                                \n1-10 author is not important\
                                \n11-20 No pain in reading other similar books\
                                \n 21-30 I like this autor, give some importance to books from this author\
                                \n 31-40 I like this author and writting sytle, give lttle more importance to this author\
                                \n 41-50 I'm Die Hard Fan to this author, very much passionate in reading"))  '''
            res[i - 1] = 30
        return res
